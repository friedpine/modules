...
for id in ids: 
	...
	if right_transcs == ():
		continue
	cursor.execute("select chr,start,start from "+table15+" where rec = 'M_G' and id = %s and chr = %s and rec = 'M_G'",[id,t[2]])
	...
	for i in left_transcs:
		if i[0] in temp['r'] and i[0] in temp['m']:
			...
			length2 = t1[5]-t[5]
			consen = (i[0],min(i[1],temp['r'][i[0]]),max(i[1],temp['r'][i[0]]),temp['m'][i[0]],length1,length2,length1+length2)
			break
	if consensus == 'NA':
		continue
